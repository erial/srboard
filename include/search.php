...
$acook = $cook.$gp;
//if($_SESSION[$acook]) unset($_SESSION[$acook]);
if($gp > 1) {
$cookp = $cook.($gp - 1);
$cookb = $cook.($gp - 2);
if(_SESSION($cookp) && (!_SESSION($cookb) || $_SESSION[$cookb] != $_SESSION[$cookp])) {
...
global $keyw,$keywc;
$return = false;
for($i = 0;$i <= $keywc;$i++) {if($keyw[$i]) {
if($keyw[$i][0] == '-') {
if(stripos($word,substr($keyw[$i],1)) !== false) $return = false;else $return = true;
} else if($_GET['search'] == 'n') {if(strcasecmp($word,$keyw[$i]) == 0) $return = true;else $return = false;
} else $return = stripos($word,$keyw[$i]);
if($return === false) break;
}}
return $return;
}
$limit = $isnt + 1;
if($gp > 1 && (!_VAR($cookp) || !_SESSION($cookp))) $slimit = ($gp-1)*$isnt;else $slimit = '';
$ii = 0;
...
$h = 1;
	...
	$fno = fgets($fn);
	if($fno == "" || $fno == "\n") {
	if($fl) list($ida,$fn,$fl) = data6($ida,array($fn,$fl),0);
	else list($ida,$fn) = data6($ida,array($fn),0);
	$h = 1;
	if($ida == 'q') break;
	} else {
	if($wnw == 2 && _VAR($bcook) && $ida == $bcook[0] && $h < $bcook[1]) {}
	else {
	if(ckcmd($fno,$flo)) {
	$o[$ii] = substr($fno,0,6);
	...
	if($wnw == 2 && $ii == $limit) {if($_GET['m']) $sum = "+";$_SESSION[$acook] = $ida."_".$h;break;}
	$i++;
	$h++;
	}}
	if($fl) fclose($fl);
	fclose($fn);
if(_VAR($bcook)) $ida = $bcook[0];
}
$i = 1;
$h = 1;
$m = "";
...
$ii = 0;
	...
	if(!$o || in_array($uuo,$o)) {
	if(!$m || !in_array($uuo,$m)) {
	if(($_GET['c'] && $_GET['c'] == (int)substr($uuu, 24, 5)) || ($_GET['search'] == 'r' && (substr($uuu, 29, -1) == $_GET['keyword'] || fkeyw(substr($yyy,24)) !== false)) || ($_GET['search'] == 'rip' && trim(substr($yyy,0,15)) == $_GET['keyword'])) {
	$m[$ii] = $uuo;
	if($slimit) $slimit--;else $ii++;
	}}
	}}
	if($ii == $limit) {$sum = "+";$_SESSION[$acook] = $ida."_".$h;break;}
	$i++;
	$h++;
	}
	}
...
if($_GET['arrange'] == 'hot') {
	...
	if(!_VAR($m[$frn]) || $frv > $m[$frn]) $m[$frn] = $frv;
	}}
	fclose($fr);
if(_VAR($bcook)) $ida = $bcook[0];
}
	...
	if(ckcmd($fno,$flo)) {
	if($_GET['arrange'] == 'date' || ($_GET['arrange'] == 'hot' && !_VAR($m[$i]))) $m[$i] = substr($flo,0,10);
	...
	fclose($fx);
  } else $sum = "+";$_SESSION[$acook] = $ida."_".$h;break;
	}
	$i++;
	$h++;
	}}

...
$srken = str_replace("<#bd_id#>",$mid,$srken);
$srken = str_replace("<#bd_name#>",$mwth[1],$srken);
if($mss[23] != '4' || $i == 0) {
$tcell = strrchr($sectgt[$f],'<td');
if(($wpx = strpos($tcell,"px'")) < 15) $wtdh = substr($tcell,11,$wpx-11);
...
$srkiin .= "<div class='tab_head thead{$on}' id='tabhead_{$iii}' onclick='rhref(this.firstChild.firstChild.href)' onmouseover='tabview(this)'><div{$first}><a href='?id={$mid}'>{$mwth[1]}</a></div></div>";
...
$srken = str_replace("<#bdid#>",$mid,$srken);
...
$nrp = (int)$zzz[2];if($mwth[9][11]) $nrp += (int)$zzz[3];if($mwth[0][49] != '2') $nrp += (int)$zzz[4];
...
else if(($zzz08 <= $mbr_level) || ($mn && $mn == $mbr_no) || (!$mn && $_COOKIE["scrt_".$no.$id] == md5($no."_".$sessid.$id))) $islock = 'unlock';
else {$secret = 2;$islock = 'lock';}
}
if($secret == 2 || $mss[63] != 4) $mmb = '';
$jsurl = 0;
if($mwth[5][4] == 1 && $mbread && $secret != 2 && $flo[5]) {
	$jsurl = "nwopn('{$flo[5]}')";
	$url = $flo[5];
	$urlp = " target='_blank'";
	if($mss[22] == 4) $srkeen = str_replace("<#isnlink#>","<;>",$srkeen);
	$mmb .= "<div class='dv_ea'>링크주소로 연결 됩니다</div>";
} else if($secret == 2 || ($secret && $mss[63] != 4)) {
if($mss[16] == 'd') $flo[3] = '[비밀글]';
$mmb = '';
} else if($mss[16] == 'd') $flo[3] = strcut($mmb, 128);
if(!$secret && $nrp && $mss[61] == 4) $srkeen = str_replace("<#ispvrp#>","<a href='?id={$mid}&amp;rp_view={$no}' class='rp'>",$srkeen);
else $srkeen = str_replace("<#ispvrp#>","<a href='#none' class='rp'>",$srkeen);
...
else if($flo[5]) {$srkeen = str_replace("<#rlink#>",$flo[5],$srkeen);$wdtt += 25;}
...
if(substr($flo[4],0,1) == "/") $flo[4] = "exe.php?sls=".$mid.$flo[4];
else if(strpos($flo[4],"exe.php") === false) $flo[4] = "exe.php?sls=".$mid."/file/".str_replace(" ","+",$flo[4]);
}
$url = "?id={$mid}&amp;no={$no}";
$flo[3] = andamp($flo[3]);
...
if($flo[4] && $mss[23] == '3') $flo[4] = '';
if($secret != 2 && $mss[18] == 4) {
$memb .= "\npretxt[{$iii}] = [\"".str_replace('"','\\"',$flo[4])."\",\"\",\"".str_replace('"','\\"',$flo[1])."\",\"".(int)$nrp."\",\"".str_replace('"','\\"',$mmb)."\"];";
$urlp .= " name='pv{$iii}'";
} else $urlp .= " name='pv00'";
if(!$gthumb && $flo[4] && $mss[23] == '4') {if(!$sett[77]) $wtdh8 = "width:".($wtdh7 -124)."px";else $wtdh8 = "";$gthumb = 131;$srkjn .= "<table class='tabtb'><tr><td width='124px'><a href='{$url}'{$urlp}><img src='{$flo[4]}' class='gthumb_100' alt='' /></a></td><td style='{$wtdh8}'>";}
} else if($mss[23] == '2') $srkeen = str_replace("<#memb#>",$mmb,$srkeen);
if($jsurl) $srkeen = str_replace("<#jsurl#>","onclick=\"{$url}\"{$urlp}",$srkeen);
else $srkeen = str_replace("<#jsurl#>","onclick=\"rhref('{$url}');\"{$urlp}",$srkeen);
...
$wdtt4[$mid] = '';
if($gthumb) $srkjn .= $srkkn."</td></tr></table></div></td></tr></table>";
else $srkjn .= $srkkn."</div></td></tr></table>";
$srkkn = '';
$gthumb = 0;
}
...
if($inclwt=inclvde($srkiin)) foreach($inclwt as $inxv) {if($inxv[0] == 1) eval($inxv[1]);else if($inxv[0] == 2) {if($sett[92][0] == '2' || $sett[92][0] == '3') $gmtime = getmicrotime();include($inxv[1]);if($sett[92][0] == '2' || $sett[92][0] == '3') echo "<!--".$inxv[1]." 처리시간:: ".(getmicrotime() - $gmtime)." -->";}else echo $inxv[1];}
$i = 0;
unset($rlmt);
?>
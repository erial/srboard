...
copy($plla."@@",$plla);
unlink($plla."@@");
echo "<script type='text/javascript'>location.replace('?no={$fda}');</script>";exit;
} else if($mbrmbr[2] == '9' && $_GET['stopvote']) {
while($time - @filemtime($plla."@@") < 3) {usleep(50000);$time = time();}
$fta = fopen($plla."@@","w");
if($fa = @fopen($plla,"r")) {
while(!feof($fa)) {$fao = fgets($fa);if(substr($fao,0,5) == $_GET['stopvote']) fputs($fta,"\x1b".$fao);else if(substr($fao,0,6) == "\x1b".$_GET['stopvote']) fputs($fta,substr($fao,1));else fputs($fta,$fao);}
fclose($fa);
}
fclose($fta);
copy($plla."@@",$plla);
unlink($plla."@@");
echo "<script type='text/javascript'>location.replace('?page=1');</script>";exit;
...
unlink($pllc."@@");
if($exit == 1) {
if($_POST['from'] == '1') echo "<script type='text/javascript'>location.replace('?no={$_POST['poll']}');</script>";
else echo "popup('include/poll.php?no={$_POST['poll']}',640,480);";
} else {
if($_POST['from'] == '1') echo "<script type='text/javascript'>alert('이미 투표하셨습니다');location.replace('?no={$_POST['poll']}');</script>";
...
if($fao[0] == "\x1b") {$novote = 1;$fao = substr($fao,1);}
$faa = explode("\x1b",$fao);
$fbb = explode("\x1b",$fbo);
...
<h4><?php echo (int)substr($faa[0],0,5)?>. <?php echo substr($faa[0],5)?> (<?php echo $apoll?>)</h4>
<ol>
<?php
for($i =1;$faa[$i +1];$i++) {
if($apoll) $percentage = sprintf("%.2f",$fbb[$i]*100/$apoll);
?>
<li><label><input type='radio' name='sub_<?php echo $plxo?>' value='<?php echo $i?>' /><?php echo $faa[$i]?> (<?php echo $fbb[$i]?>)</label><br /><div class='graph' style='width:<?php echo $percentage?>%'><?php echo $percentage?>%</div></li>
<?php
}
?>
</ol>
<?php if(!$novote) {?>
<input type='button' class='srbt' value='투표' onclick='votepoll("sub_<?php echo $plxo?>")' style='margin-left:50px;margin-bottom:10px' />
<?php } if($mbrmbr[2] == '9'){?><input type='button' class='srbt' value='투표 <?php echo ((!$novote)?"정지":"재개")?>' onclick='if(confirm("이 설문의 투표를 <?php echo ((!$novote)?"정지":"재개")?>하시겠습니까")) location.href="?stopvote=<?php echo $plxo?>"' style='margin-left:50px;margin-bottom:10px' />
<input type='button' class='srbt' value='설문 삭제' onclick='if(confirm("이 설문지를 삭제하시겠습니까")) location.href="?dellpoll=<?php echo $plxo?>"' style='margin-left:50px;margin-bottom:10px' />
<?php }?>
</div></div>
<?php
	}}

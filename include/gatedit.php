<?php
if(basename($_SERVER['PHP_SELF']) !== 'admin.php') exit;
if(!$_GET['ectgt']) {
function srwidth($stbo,$frwdth) {
global $sett;
$sett78 = ($sett[39] > $sett[78])? 0:$sett[78] - $sett[39];
$sett67 = ((int)$stbo[2])? (int)$stbo[2]:(int)$sett[67];
$sett68 = ((int)$stbo[3])? (int)$stbo[3]:(int)$sett[68];
if($sett67 && strpos($stbo[0],"@L:") !== false) {$frwdth -= $sett67;$frwdth -= $sett78;}
if($sett68 && strpos($stbo[0],"@R:") !== false) {$frwdth -= $sett68;$frwdth -= $sett78;}
return $frwdth;
}
function layoutwidth($layout) {
$srpdg = 0;
if($ectlt = @fopen("module/".$layout.".php","r")) {
for($e = 1;$e < 5;$e++) fgets($ectlt);
$ectltv = trim(fgets($ectlt));
fclose($ectlt);
if(substr($ectltv,1,5) == 'srpdg') {eval($ectltv);}}
return $srpdg;
}
function cellwidth($cwectg,$cwwdth) {
global $sett;
if($cwectg = trim($cwectg)) {
$tr = explode("<tr",$cwectg);
$cwectg = $tr[0];
$wtd = 0;
$colgroup = substr($tr[0],0,strpos($tr[0],'</colgroup>'));
if($colgroup) {
if(preg_match_all("`width='([0-9\.]+)%`i",$colgroup,$colwt)) $wtd = count($colwt[0]);
else if(preg_match_all("`width='([0-9\.]+)px`i",$colgroup,$colwp)) {
$wtd = count($colwp[0]);$test = 0;for($i=0;$i< $wtd;$i++) {$test += $colwp[1][$i];}
if($test != $cwwdth) {echo "셀의 넓이 총합이 게시판넓이와 다릅니다.";exit;}
}}
...
else $wtdh = $colw*$cwwdth/100 - ($cwth*$cols) + ($cols-1)*$sett[39];
$wtdh = round($wtdh,1);
$cwectg .= "<td width='{$wtdh}px'".$td[$d];
}
}
}
$cwectg = preg_replace("/>[\s]+</","><",$cwectg);
return $cwectg;
}
...
$srwdth = $sett[12];
if($stb = @fopen($dxr."section_arr.dat","r")) {
for($sb=1;$sb < $_POST['ectn'];$sb++) fgets($stb);
$stbo = explode("@@",fgets($stb));
fclose($stb);
$srwdth = srwidth($stbo,$srwdth);
}
$srwdth -= layoutwidth($_POST['ectl']);
$ectg = cellwidth($ectg,$srwdth);
}
$fstadd = $dxr."section_add.dat";
$tstadd = $dxr."section_add.dat@@";
$sta = fopen($fstadd, "r");
$tsta = fopen($tstadd, "w");
...
if($_GET['stgate'] || $dxrmdfd) {
if($sett[77] != '1') {
if($stb = @fopen($dxr."section_arr.dat","r")) {
if($_GET['stgate'] == 'all') $dxrmdfd = 'all';
$srpdgg = layoutwidth($sett[26]);
...
$stbg = fgets($stb);
$fstg = fgets($fst);
if($dxrmdfd == 'all' || $dxrmdfd == $i) {
if($stao = trim($stao)) {
$sectt = explode("\x1b",$fstg);
if($sectt[1] != '3' && $sectt[1] != '6' && $sectt[1] != '7' && $sectt[1] != 's') {
$stbo = explode("@@",$stbg);
$srwdth = srwidth($stbo,$sett[12]);
if(!$sectt[6]) $srwdth -= $srpdgg;
else if((int)$sectt[6] >= 1) {
$sef = fopen($dxr."section_group.dat","r");
for($ii=1;$ii < $sectt[6];$ii++) fgets($sef);
$sefg = explode("\x1b",fgets($sef));
fclose($sef);
$srwdth -= layoutwidth($sefg[1]);
}
$stao = cellwidth($stao,$srwdth)."\n";
} else $stao = "\n";
} else $stao = "\n";
}
fputs($tsta,$stao);
...
if($_GET['stgate']) echo "<body onload=\"setTimeout('self.close()', 500)\" bgcolor='#F0F0F0'><div style='font-size:100px;text-align:center'>완료</div>";
}} else if($_GET['ectgt']) {
?>
...
<textarea id='ectg' name='ectg' style='width:98%;height:180px;overflow:auto;line-height:140%'><?php echo $sectgt?></textarea>
...
<input type='button' value=' 아래로 ' onclick='toifr()' class='button' style='width:60px' />
 ...
 &nbsp; <input type='button' value=' 닫기 ' onclick='self.opener=self;window.close()' class='button' style='width:60px' />
 ...
 &nbsp; <input type='button' value=' 캐시삭제 ' onclick='ifrw.location.replace("?renewcache=<?php echo $_GET['ectgt']?>")' class='button' style='width:60px' />
</center>
</form>
...
if(mw - parseInt(dialogWidth) > 100) {dialogWidth=mw +'px';dialogHeight=(mh-70) +'px';tval = 1;}else{dialogWidth='507px';dialogHeight='437px';dialogTop=(mh-437)/2;dialogLeft=(mw-507)/2;tval = 2;}
} else {
if(mw - window.innerWidth > 100){resizeTo(mw,mh);tval = 1;if(navigator.appVersion.indexOf('Chrome') == -1) moveTo(window.screen.width-mw,0);}else{resizeTo(515,526);tval = 2;if(navigator.appVersion.indexOf('Chrome') == -1) moveTo((mw-515)/2,(mh-526)/2);}
}
if(tval == 1) {
that.value='창작게';
var nh = (mh-120)/2;
